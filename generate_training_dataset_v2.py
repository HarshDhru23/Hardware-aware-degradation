#!/usr/bin/env python3
"""
Generate and save complete training-ready dataset to disk (Version 2).
...
The generated dataset follows the exact same rules as DegradationDataset:
...
def save_hr_sample(sample: Dict, output_path: Path) -> None:
    """
    Save HR augmented image with metadata.
...
def save_lr_sample(sample: Dict, lr_idx: int, output_path: Path) -> None:
    """
    Save single LR frame with full metadata.
    ...
    save_dict = {
        ...
        'original_filename': sample['metadata']['filename'],
        'augmentation_id': sample['metadata']['aug_idx'] + 1,  # 1-based
        'lr_frame_id': lr_idx + 1,  # 1-based (1-4)
        'rotation': sample['metadata']['rotation'],
        'flip': int(sample['metadata']['flip']),
...
def get_base_filename(filepath: Path) -> str:
    ...
    Returns:
        Base filename without extension (e.g., 'PAN_AOI_2_Vegas_img1')
    """
...
def generate_dataset(input_dir: str,
    ...
    """
    Generate and save the complete training dataset in flat structure.
    ...
    Args:
        ...
        num_workers: Number of workers (currently not used)
    ...
    logger.info("="*80)
    logger.info("GENERATING TRAINING-READY DATASET (FLAT STRUCTURE)")
    logger.info("="*80)
    ...
    logger.info(f"\nDataset initialized:")
    logger.info(f"  HR images: {num_hr_images}")
    ...
    dataset_info = {
        ...
        'naming_format': {
            ...
            'lr_frames': '{basename}_aug{N}_lr{M}.npz (M=1-4)'
        },
    ...
    logger.info(f"Total files to generate: {num_augmentations * num_hr_images} HR + {num_augmentations * num_hr_images * num_lr_frames} LR")
    ...
    logger.info(f"Total files created: {stats['hr_files_created'] + stats['lr_files_created']}")
    ...
    if num_hr_images > 0:
        ...
        logger.info(f"Augmented HR:  {example_base}_aug1.npz ... {example_base}_aug8.npz")
        logger.info(f"LR frames:     {example_base}_aug1_lr1.npz ... {example_base}_aug1_lr4.npz")
        logger.info(f"               {example_base}_aug2_lr1.npz ... {example_base}_aug2_lr4.npz")
        logger.info(f"               ...")
...
def verify_dataset(output_dir: str, num_samples: int = 3, logger: logging.Logger = None) -> bool:
    ...
    Args:
        ...
        num_samples: Number of images to verify (will check their augmentations)
        logger: Logger instance
    ...
    for hr_filename in sample_files:
        ...
        # Check original file (optional, may not exist if --no_copy_originals was used)
        ...
        for aug_idx in range(1, num_augmentations + 1):
            ...
            try:
                ...
                logger.debug(f"  {hr_aug_path.name}: HR shape={hr_shape}, aug_id={aug_id}")
                
                # Check corresponding LR frames
                for lr_idx in range(1, num_lr_frames + 1):
                    ...
                    logger.debug(f"    {lr_path.name}: LR shape={lr_shape}, frame_id={lr_frame_id}, shift=({shift_x:.4f}, {shift_y:.4f})")
                    
            except Exception as e:
                logger.error(f"  Failed to load {hr_aug_path.name}: {e}")
                return False
        
        logger.info(f"  ✓ All {num_augmentations} augmentations with {num_lr_frames} LR frames verified")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate training-ready dataset in flat structure (Version 2)"
    )
    ...
    logger.info(f"    - LR frames:    {{basename}}_aug{{N}}_lr{{M}}.npz (M=1-4)")
